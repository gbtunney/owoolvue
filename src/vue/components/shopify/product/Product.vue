...
<script type="text/javascript">
	...
	export default {
        ...
        created: function() {
            ...
            ///   console.log("THE ROUTE IS!!!:",this.$route.query.variant) // outputs 'yay'
            ///console.log ("current pro: ", this.currentproduct);
            // console.log("windwow", window[this.currentproduct]);
            if (window[this.currentproduct]){
                // alert();
                let myPayload = {
                    products: [window[this.currentproduct]],

                    variantid: this.$route.query.variant,
                    productid: this.productid,
                }

                ///TODO : fix this nonsense
                this.getProduct({params: {id: this.producthandle}}).then(function(res) {
                    payload = Object.assign(myPayload);
                    payload.products = [res.data.product]
                    store.dispatch('SHOPIFY_DATA_INIT', payload).then(function(res) {
                        if (myPayload.productid){
                            store.dispatch('SET_CURRENT_PRODUCT', myPayload).then(function(res) {
                                store.dispatch('SET_CURRENT_VARIANT', myPayload);
                            });
                        }
                    })
                })
            }
            ////TODO: Ifvarables arent present do getproduct.
        },
		mounted: function() {
